...
internal class TrackingListTimeOutViewHolder(
    parent: ViewGroup,
    private val fragment: Fragment
) : BaseTrackingViewHolder(
    parent,
    R.layout.vh_child_tracking_time_out
) {
    ...
    private var model: TrackingListTimeOutUiModel? = null
    ...
    override fun onBindView(model: BaseTrackingUiModel) {
        if (model !is TrackingListTimeOutUiModel) return
        this.model = model
        val item = model.item
